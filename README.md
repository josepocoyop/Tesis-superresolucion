...
- **Figura temporal regenerada** (`fig5_rife.png`) con `ColCom Paper/reproducibility/pipeline/step8_temporal_figure.py`. La anterior interpolaba entre dos de los 60 fotogramas que step2 reparte por toda la grabación, que están separados unos 65 fotogramas; no eran contiguos y por tanto no ilustraban la interpolación temporal. Ahora usa los fotogramas 1269 y 1270 del video, separados 66.7 ms.
- **Figuras 4 y 5 fusionadas en una sola** (`fig4_comparison.png`, cuadrícula de 2x4: fotogramas completos arriba, región ampliada abajo). La fila superior de la antigua Fig. 5 repetía casi exactamente la Fig. 4. El artículo pasó de seis figuras a cinco.
- **Brecha de reproducibilidad cerrada.** Las Figs. 4 y 5 del artículo enviado se habían generado con `Archive/run_pipeline.py`, que está fuera del repositorio, mientras que `reproducibility/pipeline/step6_generate_figures.py` producía una versión distinta de tres paneles. O sea que las dos figuras principales no se podían regenerar desde el repo. El step6 ya genera exactamente la figura publicada.
- **Resolución corregida.** Las Figs. 4 y 5 estaban a 234 DPI efectivos sobre el ancho de dos columnas, por debajo de los 300 DPI que pide IEEE para contenido fotográfico. `FIG_DPI` pasó de 300 a 400 y la figura fusionada queda a 400 DPI efectivos. También se eliminó la franja vacía que quedaba entre las dos filas, causada por una altura de celda que no seguía la proporción de las imágenes.
- **Aclaración sobre lo que recupera la red.** La región ampliada cae sobre la reja de una ventana, donde ESRGAN genera barras verticales continuas mientras el original tiene una retícula de rectángulos. El texto lo dice de forma explícita: la red restituye detalle de alta frecuencia con apariencia coherente, no necesariamente el que había en la escena, lo que explica el PSNR más bajo y acota el valor forense de la salida.
- **Viñetas con material real en los diagramas.** Las Figs. 1 a 3 llevaban rectángulos blancos vacíos donde va un fotograma; ahora llevan recortes reales de `day_street.mp4` (entrada degradada, salida de ESRGAN, referencia HR y el triplete temporal). Las genera `reproducibility/pipeline/step9_diagram_thumbs.py` en `figures/thumb_*.png`, así que también son reproducibles.
- **Diagramas revisados contra el código, no contra la descripción del paper.** La Fig. 3 decía "Fusión y refinamiento", pero en el checkpoint RIFE HDv3 que se usa la etapa de refinamiento con UNet está comentada en `IFNet_HDv3.py`: lo que hay es una mezcla ponderada por una máscara que IFNet produce junto con el flujo, y esa máscara no aparecía en el diagrama. Ahora sí. También se verificó en `IFNet_HDv3.py` que `scale_list=[4,2,1]`, o sea tres escalas, y en `RRDBNet.forward` que el muestreo ascendente son dos pasos de ×2 y que hay dos convoluciones de salida, no una.
- **Más información en los diagramas.** Fig. 1: tiempo por fotograma de cada etapa y el conteo de fotogramas ($N$ a la entrada, $2N-1$ a la salida), que es lo que explica por qué se duplica la tasa. Fig. 2: 64 canales en los bloques RRDB y el muestreo en dos pasos. Fig. 3: las tres escalas de IFNet y la máscara de fusión.
- **Rótulos recolocados.** En la Fig. 1 la anotación de resolución intermedia se montaba sobre el borde del bloque ESRGAN; los tres rótulos se alinean ahora en una línea base común por debajo de los bloques. En la Fig. 2 la etiqueta "LR" caía sobre el borde discontinuo del generador.
- **Repaso de redacción.** Se quitaron expresiones que sobrevendían el resultado ("capacidad superior", "texturas fotorrealistas superiores", "balance superior", "trabajo seminal", "transformó el paradigma") y se redujeron muletillas repetidas ("de manera" pasó de 11 a 5 usos, "Conviene" de 3 a 0).
