...
\begin{table}[t]
    ...
    \begin{tabular}{lccccc}
        \toprule
        \textbf{Método} & \textbf{PSNR$\uparrow$} & \textbf{SSIM$\uparrow$} & \textbf{LPIPS$\downarrow$} & \textbf{fps} & \textbf{ms/fot.} \\
        \midrule
...
En cuanto al costo computacional, ESRGAN tarda 45.5~ms por fotograma y RIFE 14.0~ms por cada fotograma sintetizado, medidos sobre la GPU RTX~4070~Ti con el modelo ya cargado en memoria. Como RIFE genera un fotograma por cada par de entrada, el flujo completo consume 52.5~ms por fotograma de entrada, equivalentes a 19.0~fps sostenidos. Al resolverse en $212\times120$~píxeles, el pico de memoria de video es de 360~MB para ESRGAN y 274~MB para RIFE, y como las etapas se ejecutan de forma secuencial el flujo se mantiene por debajo de 400~MB. El sistema procesa por tanto una cámara de 15~fps en tiempo real sobre una GPU de escritorio, sin necesidad de infraestructura en la nube, aunque la cifra deja poco margen para atender varias cámaras en paralelo con el mismo equipo.
...
La evaluación cuantitativa sobre 60~fotogramas muestra que ESRGAN obtiene un LPIPS de 0.311 frente a 0.714 del escalado bicúbico, una reducción del 56~\%, con una diferencia de SSIM marginal (0.5194 frente a 0.5132) y un PSNR 1.58~dB inferior. Este comportamiento es consistente con la brecha percepción-distorsión~\cite{blau2018perception}: el generador sacrifica fidelidad píxel a píxel para producir texturas visualmente coherentes que la bicúbica no recupera. En el dominio temporal, la prueba de exclusión sobre 23~tripletes con movimiento sitúa los fotogramas sintetizados por RIFE en 37.12~dB frente a 33.95~dB del promediado de vecinos. El flujo completo se ejecuta en 52.5~ms por fotograma de entrada y menos de 400~MB de memoria de video en una GPU de escritorio, lo que permite sostener la tasa de captura de la cámara sin recurrir a infraestructura externa. Los resultados se obtuvieron sobre una única escena diurna, por lo que constituyen una caracterización inicial y no una validación en condiciones variadas de operación.
